...
} from "@/utils/supabase/email-helpers";
...
app.get("/make-server-fcebfd37/health", (c) => {
  return c.json({ status: "ok" });
});

// User registration endpoint
app.post("/make-server-fcebfd37/signup", async (c) => {
  try {
    ...
    if (!["auditor", "auditee", "manager"].includes(role)) {
      return c.json({ error: "Invalid role" }, 400);
    }

    // Validate @ecobank.com email domain for all roles
    ...
    if (pendingRequests.length > 0) {
      console.log(
        `Auto-assigned ${pendingRequests.length} pending requests to user ${email}`
      );
    }
...
app.post("/make-server-fcebfd37/requests", async (c) => {
  try {
    ...
    if (assignedUser) {
      ...
      // Store as pending assignment - will be resolved when user signs up
      console.log(`Creating request for future user: ${assigned_to_email}`);
    }
    ...
    const request = {
      ...
      status: "submitted",
      created_by: user.id,
      assigned_to: assignedUserId, // null if user doesn't exist yet
      assigned_to_email,
      department,
      cc_emails: cc_emails || [],
      pending_assignment: !assignedUserId, // flag to indicate this needs assignment when user signs up
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
    };
    ...
    try {
      const result = await triggerNewRequestEmail(
        ...
        userProfile, // auditor
        null // manager not always needed here
      );
    ...
    if (userProfile.role === "auditor") {
      ...
      filteredRequests = allRequests.map((req) => {
        if (req.department === "Human Resources") {
          // For HR requests, hide sensitive response data from auditors
          return {
            ...req,
            hr_confidential: true, // Flag to indicate this is a confidential HR request
          };
        }
        return req;
      });
      ...
      // Auditees only see requests assigned to them (including by email if not yet assigned by ID)
...
app.put("/make-server-fcebfd37/requests/:requestId/status", async (c) => {
  try {
    ...
    if (
      !["submitted", "in_progress", "rejected", "approved"].includes(status)
    ) {
    ...
    if (userProfile?.role !== "auditor" && userProfile?.role !== "manager") {
      return c.json(
        { error: "Only auditors and managers can update request status" },
        403
      );
    }
    ...
    try {
      const result = await triggerStatusChangeEmail(
        ...
        userProfile, // auditor or manager who updated
        null // optional: manager profile if needed
      );
...
// Verify OTP and complete signup (auditee, auditor, or manager)
app.post("/make-server-fcebfd37/verify-otp-signup", async (c) => {
  try {
    ...
    return c.json(
      {
        error: "Internal server error while verifying OTP/signup",
      },
      500
    );
  }
